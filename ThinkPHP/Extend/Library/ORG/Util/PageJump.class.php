	...
	public function show_ex() {
		...
		$pageStr = '<script type="text/javascript">
			function PageEnterPress(obj, e){
				...
				if(e.keyCode == 13){
					window.location.href = "'.str_replace('__PAGE__','"+nowPage+"',$url).'";
				}
			}
			function pageJump(selObj) { //v3.0
				window.location.href = "'.str_replace('__PAGE__','1',$url).'";
			}
		</script>';
        // 组装分页字符串
        ...
        if($this->nowPage != 1) {
        	$pageStr .= '<a href="'.str_replace('__PAGE__',$upRow,$url)."&numPerPage=".$_REQUEST['numPerPage'].'" class="prev">上一页</a>';
        } else {
        	$pageStr .= '<span class="disabled" >上一页</span>';
        }
        $pageStr .= ' &nbsp; &nbsp;第&nbsp;<input type="text" maxlenght="5" value="'.$this->nowPage.'" style="line-height:normal; vertical-align:baseline;" onkeypress="PageEnterPress(this, event)" onkeydown="PageEnterPress(this, event)"/>&nbsp; 页&nbsp;&nbsp;';
        
        if($this->nowPage != $this->totalPages) {
        	$pageStr .= '<span><a href="'.str_replace('__PAGE__',$downRow,$url)."&numPerPage=".$_REQUEST['numPerPage'].'" class="next">下一页</a></span>';
        } else {
        	$pageStr .= '<span class="disabled">下一页</span>';
        }
        $pageStr .= '&nbsp;&nbsp;| &nbsp;&nbsp;共 '.$this->totalPages.' 页 ';
        return $pageStr;
	}
	...
	/* public function show_ex() {
		...
		$pageStr .= '每页显示 &nbsp;
					<select class="form-control" style="width:75px; float:none;" id="pageJumpSelect" onchange="javascript:pageJump(this);">
						...
						<option value="10" '.($_REQUEST['numPerPage'] == '10' ? 'selected="selected"' : '').'>10</option>
