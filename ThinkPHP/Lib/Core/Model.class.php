...
class Model {
	...
	public function add($data='',$options=array(),$replace=false) {
		...
		// 查看数据表是否有id字段   add by xinglou 2014-7-1
		...
		// 写入数据到数据库
		$result = $this->db->insert($data,$options,$replace,$has_id);
		if(false !== $result ) {
	...
	public function addAll($dataList,$options=array(),$replace=false,$has_id=true){
		if(empty($dataList)) {
			$this->error = L('_DATA_TYPE_INVALID_');
			return false;
		}
		...
		// 写入数据到数据库
		$result = $this->db->insertAll($dataList,$options,$replace,$has_id);
		if(false !== $result ) {
			$insertId   =   $this->getLastInsID();
			if($insertId) {
				return $insertId;
	...
	public function save($data='',$options=array()) {		
		...
		$options['enableLog'] = $this->enableLog;
